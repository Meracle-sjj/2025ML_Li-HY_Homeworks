...
class TMFModel(pl.LightningModule):
    def __init__(self, args):
        ...
        # 将raw data embedding到latent space
    ...
    def init_args(parent_parser):
        #和数据处理相关的部分，这里设计了和数据处理相关的参数，构成的方式是直接拼接
        root_path = "/home/shijunjian/data/datasets/"
        parser_dataset = parent_parser.add_argument_group("dataset")
        parser_dataset.add_argument(
            "--train_split", type=str, default=os.path.join(
                root_path, "train"))
        # 例如 --train_split = /home/shijunjian/data/datasets/train
        # 这里的路径是相对路径，和上面拼接的路径拼接在一起
        parser_dataset.add_argument(
